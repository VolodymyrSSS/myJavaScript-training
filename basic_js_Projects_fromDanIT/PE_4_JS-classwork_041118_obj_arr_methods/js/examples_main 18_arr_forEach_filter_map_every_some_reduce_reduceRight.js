...
// где: item – очередной элемент массива.   i – его номер.   arr – массив, который перебирается.
// Например:
let arr = ["Яблоко", "Апельсин", "Груша"];
arr.forEach(function(item, i, arr) {
 alert( i + ": " + item + " (массив:" + arr + ")" ); // выведет по очередно: 0: Яблоко (массив:Яблоко,Апельсин,Груша)  1: Апельсин (массив:Яблоко,Апельсин,Груша)  2: Груша (массив:Яблоко,Апельсин,Груша)
});
// Второй, необязательный аргумент forEach позволяет указать контекст this для callback. Мы обсудим его в деталях чуть позже, сейчас он не важен
...
//  return number > 0;
// });
// alert( positiveArr ); // выведет: 1,2,3
...
//  return name.length;
// });
// alert( nameLengths ); // 4,3,10 - получили массив с длинами
...
//   return number > 0;
// }
// alert( arr.every(isPositive) ); // выведет: false - не все положительные
// alert( arr.some(isPositive) ); // выведет: true - есть хоть одно положительное
...
let strLongest = (str) => {                                       // створюємо функцію під назвою strLongest яка буде повертати елемент-рядок 
  ...
  let arr = str.split(" ");                                       // створюємо масив якому присвоюємо значення отримані в результаті перетворення рядка у масив методом str.split і роздільником- пробіл 
    for(let i = 0, arrLength = arr.length; i < arrLength; i++){   // перебираємо елементи і масиву для пошуку найдовшого слова 
      if (result.length < arr[i].length) {                        // якщо довжина і-елемента масиву більша від довжини взятого елементу 
